...
function getPolynom(...arr) {
  ...
  if (arr.length === 2) return (x) => arr[0] * x + arr[1];
  if (arr.length === 3) return (x) => arr[0] * x ** 2 + arr[1] * x + arr[2];
  return null;
}
...
function logger(func, logFunc) {
  return (...args) => {
    ...
    logFunc(`${func.name}(${arr.slice(1, arr.length - 1)}) starts`);
    const current = func(...args);
    logFunc(`${func.name}(${arr.slice(1, arr.length - 1)}) ends`);
    return current;
  };
}
